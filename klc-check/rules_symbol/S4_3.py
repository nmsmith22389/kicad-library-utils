...
class Rule(KLCRule):
    ...
    def check(self):
        ...
        for (pos, pins) in self.component.get_pinstacks().items():
            ...
            for pin in pins:
                if pin.number_int == None and not pos in non_numeric:
                ...
                if pin.etype == 'no_connect':
                    self.error("NC {pin} is stacked on other pins".format(
                               ...
                               x=pin.posx,
                               y=pin.posy))
                    NC_stacked.append(pin)

                # Check2: all pins should have the same name
                if pin.name != common_pin_name and not pos in different_names:
                    self.error("Pin names in the stack have different names")
                    different_names.append(pos)
                    for pin in pins:
                ...
                if pin.is_hidden == False:
                    ...
                    if pin.number_int != None and pin.number_int != min_pin_number and not pos in visible_pin_not_lowest:
                        ...
                        visible_pin_not_lowest.append(pos)
        ...
        return more_then_one_visible or len(different_types) > 0 or len(NC_stacked) > 0 or len(different_names) > 0 or special_stack_err
