...
class Device:
    ...
    _FOOTPRINT_MAPPING = {
            ...
            "UFBGA59":      ("Package_BGA:ST_UFBGA-59_5x5mm_Layout8x8_P0.5mm",              "*UFBGA*5x5mm*Layout8x8*P0.5mm*"), # NOQA
            ...
            "UFBGA81":      ("Package_BGA:ST_UFBGA-81_5x5mm_Layout9x9_P0.5mm",              "*UFBGA*5x5mm*Layout9x9*P0.5mm*"), # NOQA
    ...
    _WLCSP_MAP = {
            ...
            'ST_WLCSP-99_Die415':  ('Package_CSP:ST_WLCSP-99_4.42x3.77mm_Layout9x11_P0.35mm', '*'), # NOQA
            ...
            'ST_WLCSP-42_Die489':  ('Package_CSP:ST_WLCSP-42_2.93mmx2.82_P0.40mm_Stagger', '*'), # NOQA
            'ST_WLCSP-101_Die485': ('Package_CSP:ST_WLCSP-101_3.86x3.79mm_P0.35mm_Stagger', '*'), # NOQA
            'ST_WLCSP-27_Die459':  ('Package_CSP:ST_WLCSP-27_2.34x2.55mm_P0.40mm_Stagger', '*'), # NOQA
            'ST_WLCSP-208_Die476': ('Package_CSP:ST_WLCSP-208_5.8x5.6mm_P0.35_Stagger', '*'), # NOQA
    }
...
def cli(cubemx_mcu_db_dir, part_number_pattern, output_file, verify_datasheets, url_cache,
    ...
    def try_fetch_url(url):
        ...
        if status_code in (200, 302):
        ...
        elif status_code is None or (status_code in ('curl timeout', 'curl error') and retry_datasheets):
            ...
            try:
                res = subprocess.run(['curl',
                                      ...
                                      datasheet], check=True, capture_output=True, text=True)
    ...
    for filename, variants in tqdm.tqdm(files.items(), desc=pathlib.Path(output_file).stem):
        ...
        if (not verify_datasheets or not datasheet) and \
                any(fnmatch.fnmatch(refname, pattern) for pattern in ignore_devices):
            tqdm.tqdm.write(f'Skipping known weird or non-existing device {refname}.')
            continue
