...
from kicad_sym import KicadLibrary, KicadSymbol
from kicad_sym import Pin as KicadPin
from kicad_sym import Rectangle
...
class Device:
    ...
    _FAMILY_DATASHEETS = {
        "Spartan7":
            "https://www.xilinx.com/support/documentation/data_sheets/ds180_7Series_Overview.pdf",  # noqa: E501
        "Artix7":
            "https://www.xilinx.com/support/documentation/data_sheets/ds180_7Series_Overview.pdf",  # noqa: E501
        "Kintex7":
            "https://www.xilinx.com/support/documentation/data_sheets/ds180_7Series_Overview.pdf",  # noqa: E501
        "Virtex7":
            "https://www.xilinx.com/support/documentation/data_sheets/ds180_7Series_Overview.pdf",  # noqa: E501
        "Zynq7000":
            "https://www.xilinx.com/support/documentation/data_sheets/ds190-Zynq-7000-Overview.pdf",  # noqa: E501
        "ZynqUltrascaleP":
            "https://www.xilinx.com/support/documentation/data_sheets/ds890-ultrascale-overview.pdf",  # noqa: E501
        "ZynqUltrascalePRFSoC":
            "https://www.xilinx.com/support/documentation/data_sheets/ds890-ultrascale-overview.pdf",  # noqa: E501
        "ArtixUltrascaleP":
            "https://www.xilinx.com/support/documentation/data_sheets/ds890-ultrascale-overview.pdf",  # noqa: E501
        "KintexUltrascaleP":
    ...
    class Pin:
        ...
        class PinType(IntEnum):
            ...
            PS_DDR_DCI_REF = (
                34  # PS_DDR_VR* - RAM DCI reference voltage for Zynq CPU (PS)
            )
        ...
        _TYPE_REGEXPS = {
            re.compile(
                "^((PROGRAM)|(INIT)|(DONE))|(CFGBVS)|(CCLK)|(PUDC_B)|(POR_OVERRIDE)"
            ): PinType.CONFIG,
            ...
            re.compile("^VCCO_(?:DDR_)?(?:MIO\\d*_)?(\\d+)"): PinType.BANK_POWER,
            ...
            re.compile("^PS_((CLK)|(POR_B)|(SRST_B))"): PinType.PS_CTL,
            ...
            re.compile(
                "^PS_DDR_((DRST)|(CS_B)|(CKE)|(WE_B)|(CAS_B)|(RAS_B))"
            ): PinType.PS_DDR_CTL,
        ...
        _PIN_NAME_REMAP = {re.compile("^RSVDVCC\\d*"): "VCCO_0"}
        ...
        def __init__(self, dic):
            ...
            except ValueError:  # All pins without bank goes to bank 0
                self.bank = 0
            ...
            if self.type == self.PinType.GENERAL:
                ...
                self.type == self.PinType.PS_GENERAL
                or self.type == self.PinType.PS_DDR_DQ
                or self.type == self.PinType.PS_DDR_DM
                or self.type == self.PinType.PS_DDR_A
                or self.type == self.PinType.PS_DDR_BA
                or self.type == self.PinType.PS_DDR_VREF
            ):
                self.number = int(m.group(1))
            elif (
                self.type == self.PinType.ADC
                or self.type == self.PinType.ADC_DIODE
                or self.type == self.PinType.ADC_REF
                or self.type == self.PinType.PS_DDR_CLK
                or self.type == self.PinType.PS_DDR_DCI_REF
            ):
                self.pair = True
                ...
                self.type == self.PinType.MGT_RX
                or self.type == self.PinType.MGT_TX
            ...
            if self.type in self._BANK_REMAP:
                ...
                if self.bank != newBank:
                    logging.warning(
                        f"Wrong bank for {self.name}, was {self.bank} should be {newBank}"
                    )
                    self.bank = newBank
                ...
                if self.type < other.type:
                    ...
                    if (
                        ...
                        if self.pair and other.pair and self.pairNeg < other.pairNeg:
                            ...
                            not self.pair
                            or not other.pair
                            or self.pairNeg == other.pairNeg
                        ):
                            return self.name < other.name

        def __str__(self):
            return (f"({self.name}, {self.pin}, {self.bank}, {self.bankType},"
                    f" {self.number}, {self.type}, {self.pair}, {self.pairNeg})"
                    )

        def kicadPin(self, **kwargs):
            # Try to find electrical type
            eltype = (
                self._TYPE_DEFAULT_ELTYPE[self.type]
                if self.type in self._TYPE_DEFAULT_ELTYPE
                else "passive"
            )
    ...
    def readPins(self):
        ...
        if self.pinCount != self.packagePins:
            logging.warning(
                f"{self.libraryName} - package pin count mismatch: "
                f"found {self.pinCount} pins, should be {self.packagePins}"
            )
    ...
    def parseFile(self):
        ...
        fpFile = glob.glob(
            os.path.join(FOOTPRINT_PATH, "Package_BGA.pretty/", self.footprintFilter)
        )
        if len(fpFile) > 0:
            self.footprint = (
                f"Package_BGA:{os.path.splitext(os.path.basename(fpFile[0]))[0]}"
            )

        if not self.footprint:
            self.footprint = f"Package_BGA:Xilinx_{self.packageName.upper()}"
            logging.warning(
                f"{self.libraryName} - cannot find footprint file, suggested {self.footprint}"
            )
    ...
    def drawSymbol(self):
        ...
        for bank, pins in bankList:
            for pin in pins:
                maxPackagePin = (
                    len(pin.pin) if len(pin.pin) > maxPackagePin else maxPackagePin
                )
        ...
        for bankIdx in range(0, self.symbol.unit_count):
            ...
            forcedSides = {
                "l": [
                    self.Pin.PinType.CONFIG,
                    self.Pin.PinType.BOOT,
                    self.Pin.PinType.ADC_REF,
                    self.Pin.PinType.ADC,
                    self.Pin.PinType.MGT_RX,
                    self.Pin.PinType.MGT_REFCLK,
                    self.Pin.PinType.MGT_RREF,
                    self.Pin.PinType.MGT_RCAL,
                    self.Pin.PinType.PS_MIO_VREF,
                    self.Pin.PinType.PS_CTL,
                    self.Pin.PinType.PS_DDR_DQ,
                    self.Pin.PinType.PS_DDR_DQS,
                    self.Pin.PinType.PS_DDR_DM,
                    self.Pin.PinType.PS_VCCPINT,
                    self.Pin.PinType.PS_VCCPAUX,
                    self.Pin.PinType.PS_VCCPLL,
                    self.Pin.PinType.CORE_POWER,
                    self.Pin.PinType.BRAM_POWER,
                    self.Pin.PinType.AUX_POWER,
                    self.Pin.PinType.AUX_IO_POWER,
                    self.Pin.PinType.HBM_POWER,
                    self.Pin.PinType.HBM_IO_POWER,
                    self.Pin.PinType.HBM_AUX_POWER,
                    self.Pin.PinType.BANK_VREF,
                    self.Pin.PinType.GND,
                ],
                "r": [
                    self.Pin.PinType.JTAG,
                    self.Pin.PinType.BOOT_SPI,
                    self.Pin.PinType.ADC_DIODE,
                    self.Pin.PinType.MGT_TX,
                    self.Pin.PinType.PS_DDR_VREF,
                    self.Pin.PinType.PS_DDR_CLK,
                    self.Pin.PinType.PS_DDR_CTL,
                    self.Pin.PinType.PS_DDR_A,
                    self.Pin.PinType.PS_DDR_BA,
                    self.Pin.PinType.PS_DDR_DCI_REF,
                    self.Pin.PinType.PS_DDR_ODT,
                    self.Pin.PinType.ADC_POWER,
                    self.Pin.PinType.BATT_POWER,
                    self.Pin.PinType.MGT_POWER,
                    self.Pin.PinType.MGT_AUX_POWER,
                    self.Pin.PinType.MGT_VTT,
                    self.Pin.PinType.ADC_GND,
                    self.Pin.PinType.MGT_SENSE,
                    self.Pin.PinType.MGT_THERM,
            ...
            genSide = "l"
            genPins = (
                pinsGrouped[self.Pin.PinType.GENERAL]
                if self.Pin.PinType.GENERAL in pinsGrouped
                else []
            )
            ...
            maxRowNames = (
                maxNames["l"] if maxNames["l"] > maxNames["r"] else maxNames["r"]
            )
            ...
            maxLRHeight = (
                pinoffsets["l"]
                if pinoffsets["l"] > pinoffsets["r"]
                else pinoffsets["r"]
            ) * 100
            maxTBWidth = (
                pinoffsets["t"]
                if pinoffsets["t"] > pinoffsets["b"]
                else pinoffsets["b"]
            ) * 100
            ...
            if maxRowNames > maxTBWidth:
                ...
                height = _roundUp(maxLRHeight, 200) + _roundUp(
                    _roundUp(maxNames["t"] + 70) + _roundUp(maxNames["b"] + 70), 200
                )
    ...
    def createSymbol(self, library):
        ...
        # Make the symbol
        self.symbol = KicadSymbol(
            self.libraryName, f"FPGA_Xilinx_{self.family}", library.filename
        )
        self.symbol.add_default_properties()
        self.symbol.get_property("Datasheet").value = self._FAMILY_DATASHEETS[
            self.family
        ]
        self.symbol.get_property("ki_keywords").value = f"Xilinx FPGA {self.familyName}"
        self.symbol.get_property(
            "ki_description"
        ).value = f"Xilinx {self.familyName} FPGA, {self.name.upper()}, {self.packageName.upper()}"
        self.drawSymbol()
        library.symbols.append(self.symbol)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generator for Xilinx FPGAs symbols")
    parser.add_argument(
        "dir", help="Directory containing ONLY valid Xilinx ASCII package files (CSV)"
    )
    parser.add_argument(
    ...
    for fpga in devices:
        ...
        if fpga.family not in libraries:
            libraries[fpga.family] = KicadLibrary(
                f"FPGA_Xilinx_{fpga.family}.kicad_sym"
            )
